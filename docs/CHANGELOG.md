...
- **Otimização Crítica do Escaneamento de Tabela (`Find-TableControl`):** Removida a chamada recursiva `$table.FindAll(Descendants)` que varria os mais de 400.000 elementos da tabela JavaFX. A varredura massiva estourava a memória do provedor COM accessibility (`glass.dll`), causando a queda súbita (`crash`) da JVM do Java e o fechamento automático da Santa Cruz. A validação da tabela agora é feita instantaneamente verificando a propriedade de colunas da grade (`GridPattern.ColumnCount -ge 12`).
- **Digitação Única e Direta (`Ensure-SantaCruzSearchInput`):** Reformulada a inserção de texto para executar uma única passagem de foco por clique físico de mouse + `Ctrl+A` + `Backspace` + digitação. Eliminado o loop de repetição que re-executava sequências de apagar (`Ctrl+A` + `Backspace` + `Delete`), cujas repetições em alta velocidade acionavam o atalho de fechar do JavaFX.
...
- **Bloqueio de Foco Físico de Mouse antes do Enter:** Na submissão da pesquisa, o robô clica fisicamente com o mouse no centro da caixa de texto antes de enviar qualquer `{ENTER}`, assegurando que a tecla Enter permaneça estritamente contida no campo de busca.
