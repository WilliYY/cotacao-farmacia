...
- **Remoção Completa de Teclas Globais na Leitura de Tabela (`Read-AllSantaCruzRowsWithScroll`):** Eliminados os envios de atalhos globais de teclado (`Ctrl+Home` e `PageDown`) durante a leitura da grade de resultados. Como a Santa Cruz é uma aplicação Java Swing, o envio de `Ctrl+Home` ou `Enter` quando o foco estava em uma célula da tabela ou barra de tarefas acionava o atalho nativo de "Finalizar Pedido / Fechar Janela". A leitura agora é realizada **exclusivamente via UIAutomation `GridPattern` com zero teclas enviadas à janela**, preservando a Santa Cruz 100% aberta na tela.
- **Bloqueio de Foco Físico de Mouse antes do Enter:** Na submissão da pesquisa, o robô clica fisicamente com o mouse no centro da caixa de texto antes de enviar qualquer `{ENTER}`, assegurando que a tecla Enter permaneça estritamente contida no campo de busca e nunca acione botões padrão de fechamento da janela.
- **Inserção Direta via UIAutomation (`Ensure-SantaCruzSearchInput`):** O robô agora prioriza `ValuePattern.SetValue("")` seguido de `SetValue($TargetValue)` **sem disparar atalhos globais de teclado** (`Ctrl+A`, `Backspace`, `Delete`) se a propriedade for suportada.
- **Foco por Clique de Mouse Físico:** Caso o `SetValue` direto não seja suficiente e a digitação caractere por caractere seja necessária, o robô faz um clique de mouse físico (`SetCursorPos` + `mouse_event`) exatamente dentro dos limites do retângulo da caixa de texto antes de qualquer tecla.
- **Validação Estrita de Texto Exato:** Alterada a validação do campo de busca de `.Contains()` para **igualdade exata de texto normalizado** (`$normCurrent -eq $normTarget`). Se houver duplicação (`losartana 50losartana 50`), a validação rejeita, limpa o campo do zero e reescreve o termo.
