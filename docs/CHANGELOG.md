...
- **Seleção Exata da Tabela de Busca (`Find-TableControl`):** O robô agora diferencia a tabela do **Carrinho de Pedidos** (17 colunas, coluna 0 botão lixeira ``) da tabela de **Resultados da Busca** (18 colunas, coluna 0 código EAN de 13 dígitos). Anteriormente, a automação selecionava a tabela do carrinho por engano e, ao tentar ler códigos EAN na lixeira, falhava todas as pesquisas informando *"A grade mudou, mas os produtos ainda não puderam ser lidos"*.
- **Restauração de Janela Minimizada (`SW_RESTORE`):** Adicionada chamada explícita de desminimização via `ShowWindow(handle, 9)` (SW_RESTORE) antes de calcular coordenadas físicas de cliques de mouse. Isso impede que o robô tente clicar em coordenadas off-screen negativas (`x=-31390, y=-31596`) quando a Santa Cruz estiver minimizada.
- **Otimização Crítica do Escaneamento de Tabela (`Find-TableControl`):** Removida a chamada recursiva `$table.FindAll(Descendants)` que varria os mais de 400.000 elementos da tabela JavaFX. A varredura massiva estourava a memória do provedor COM accessibility (`glass.dll`), causando a queda súbita (`crash`) da JVM do Java e o fechamento automático da Santa Cruz.
- **Digitação Única e Direta (`Ensure-SantaCruzSearchInput`):** Reformulada a inserção de texto para executar uma única passagem de foco por clique físico de mouse + `Ctrl+A` + `Backspace` + digitação. Eliminado o loop de repetição que re-executava sequências de apagar (`Ctrl+A` + `Backspace` + `Delete`).
- **Remoção Completa de Teclas Globais na Leitura de Tabela (`Read-AllSantaCruzRowsWithScroll`):** Eliminados os envios de atalhos globais de teclado (`Ctrl+Home` e `PageDown`) durante a leitura da grade de resultados. A leitura agora é realizada **exclusivamente via UIAutomation `GridPattern` com zero teclas enviadas à janela**, preservando a Santa Cruz 100% aberta na tela.
- **Proteção Tripla contra Reabertura e Fechamento do Software:** Adicionada checagem tripla de segurança antes de qualquer chamada a `Start-Process`. Se a Santa Cruz já estiver aberta, o robô **nunca** executa um segundo `Start-Process`, prevenindo o encerramento forçado da aplicação por seu monitor de instância única.
