...
- **Inserção Direta via UIAutomation (`Ensure-SantaCruzSearchInput`):** O robô agora prioriza `ValuePattern.SetValue("")` seguido de `SetValue($TargetValue)` **sem disparar atalhos globais de teclado** (`Ctrl+A`, `Backspace`, `Delete`) se a propriedade for suportada. Isso evita enviar atalhos para a janela Java Swing que podiam acionar atalhos internos de fechamento da tela.
- **Foco por Clique de Mouse Físico:** Caso o `SetValue` direto não seja suficiente e a digitação caractere por caractere seja necessária, o robô faz um clique de mouse físico (`SetCursorPos` + `mouse_event`) exatamente dentro dos limites do retângulo da caixa de texto antes de qualquer tecla, garantindo 100% de foco no campo e impedindo atalhos de janelas pai.
