...
- **Santa Cruz preflight:** `--status-only` is read-only and reports `closed`, `ready`, `updating`, `login-required`, `logged-in-home`, `logged-in-orders`, `running-without-window`, `open-not-ready`, or `not-installed`. The renderer polls this through `get-santacruz-status` every 30 seconds while Santa Cruz is selected.
- **Santa Cruz preparation:** `--prepare` reuses the same login/navigation path but stops after the live search grid is ready. Only this explicit operator command may restart a validated Santa Cruz `javaw` that has no accessible window; normal quotations remain fail-closed and never terminate the supplier application.
