...
erDiagram
    ...
    QuoteResult {
        ...
        integer farmaciaPopular
        string farmaciaPopularCategory
        string farmaciaPopularCoverage
        string farmaciaPopularNotes
    }
    SystemLog {
        integer id PK
        string level
...
- A unique short continuation can inherit its ingredient (`metformina 500` followed by `met 850`) only when the strength is compatible with that ingredient. `met 25` remains ambiguous between metformina and metoprolol and is not sent to a portal.
...
- `QueryCorrection` may record observations, but only manual approval or an official alias/typo source is trusted for a future automatic correction. It never stores or supplies price, stock, ST, or a recommendation.
- For input containing EAN plus description, the connector tries EAN first and retries by name only after a genuine empty response. Infrastructure errors stay blocked, and an exact barcode does not bypass a conflicting description supplied on the same line. For EAN-only input, an exact supplier row is the complete product identity and must not be rejected merely because the query omitted name, dose or presentation. An exact row from one supplier may also provide the catalog identity for another empty supplier; dose, presentation and package quantity/size must all be present and match, and the original EAN remains mandatory.
...
6. Sort by:
   - **Priority 1:** Audit quality (`OK` before `ATENCAO`).
   - **Priority 2:** ST Priority (prefer `COM_ST` over `ST_SEPARADO`).
   - **Priority 3:** Lowest `unitPrice` when packaging differs.
...
- Blocks invalid or zero prices, unconfirmed stock, unavailable products, `SEM_ST`, `ST_DESCONHECIDO`, EAN mismatch on barcode searches, dosage mismatch, presentation mismatch, and product names that do not fuzzy-match the search.
- Keeps supplier infrastructure failures separate from out-of-stock products. Timeout, network failure and updater failure remain technical blocks and never become `not_found`.
...
- Recalculates and persists comparable unit price from `price / quantity`; a divergent connector-provided `unitPrice` is never trusted for ranking.
- Detects price outliers across suppliers using the persisted comparable unit price so different pack sizes do not create false alarms.
...
- **ANB:** searches with EAN when available; otherwise sends name + dosage + presentation. The captured quote price must come directly from the literal `Unit c/ST` grid column. Required headers are validated before extraction, the previous grid signature cannot satisfy a new search, and missing/zero or currency-incompatible `Unit c/ST` blocks the row. An exact EAN search may confirm a missing row barcode only when the live grid has one named result with credible price and availability evidence; multiple rows and conflicting returned EANs remain blocked. Never fall back to `Preço`, `Preço + ST`, or a package calculation.
- **Profarma:** uses the Electron BrowserWindow scraper, normalizes old addresses to `https://pedido.profarma.com.br/`, opens `Novo Pedido`, and accepts only `Preço Final`. A medicine with `ST R$ -` is blocked unless its category is explicitly ST-exempt. Availability requires an enabled quantity input or a semantic add/increment button; disabled controls and `Avise-me` remain unavailable.
- **Santa Cruz:** uses `src/lib/santacruz-search.ps1` for the local JavaFX program. Discovery checks an optional environment override, a validated machine-local path cache, Desktop/Start Menu shortcuts, uninstall registry entries, standard install folders, and finally a time-bounded fixed-drive scan. It opens the app, handles login, waits for updates, enters Digitalizador/Novo Pedido, writes the medication, requires a changed live grid for every accepted price, extracts only column `Preço NF`, and requires green visual stock evidence from `Disp.`. A row must contain a 13-digit EAN, a textual product description, positive and currency-compatible `Preço NF`, ST evidence and stock evidence. A transient row that places any barcode-sized value in the price column is rejected and searched at most once more; repeated corruption becomes a blocked layout failure.
- **Santa Cruz preflight:** `--status-only` is read-only and reports `closed`, `ready`, `updating`, `login-required`, `logged-in-home`, `logged-in-orders`, `not-responding`, `running-without-window`, `open-not-ready`, `not-installed`, or `busy`. The renderer polls this through `get-santacruz-status` every 30 seconds while Santa Cruz is selected, exposes the reason and available actions, and discards an older status response when explicit preparation has already started.
...
- **Santa Cruz global mutex:** every command that can alter the supplier window (`search`, `--prepare` or `--cleanup`) acquires `Local\WimifarmaCotacaoSantaCruz`. Read-only discovery/status probes do not acquire it. A second Electron or diagnostic process receives structured status `busy` instead of typing into the same field.
...
- **DM Playwright pilot:** `DM_BROWSER_ENGINE=electron` is the rollout default. Explicit `playwright` uses the installed Microsoft Edge channel and a machine-local persistent profile under Electron user data. `auto` is reserved for controlled rollout and may select Electron only when Playwright or Edge fails before any portal interaction. No browser binary or user path is copied between computers.
- **DM retry boundary:** authentication rejection, CAPTCHA, network failure, layout mismatch, stale grid, abort and timeout never trigger a second browser engine. This prevents duplicate logins and unbounded retries. The existing connector recovery policy remains the only bounded retry layer above the browser engine.
- **DM fresh-grid proof:** before Enter, the Playwright route records the current card signature. A result is accepted only when the input equals the submitted term and either the card signature changes or the DM product API confirms a response containing that term. Unrelated cart/session traffic is ignored. Empty state must remain stable for at least 750 ms and 2.5 seconds after Enter. A later unresolved failure from the matching search request blocks the source.
- **DM delayed login and pagination:** the route waits for login fields rendered after `DOMContentLoaded`, rechecks CAPTCHA while waiting, and accepts a next page only after its non-empty result signature remains stable for 500 ms. Pagination defaults to 50 pages (`DM_PLAYWRIGHT_MAX_PAGES`, capped at 100) and fails closed instead of returning a truncated quotation.
- **DM diagnostic trace:** tracing starts only after the authenticated search field is visible. `on-failure` saves screenshots and DOM snapshots under `logs/scraper-debug`; login inputs and passwords are outside the trace window, but authenticated account content remains sensitive. Only the ten latest DM traces are retained. `always` is available for a controlled diagnostic and `off` disables traces.
...
- **Network retry and recovery:** only `retryable` transient portal failures receive the bounded connector retry (`CONNECTOR_RETRY_COUNT=1`). After `SUPPLIER_FAILURE_THRESHOLD=3` consecutive item failures, the supplier enters a half-open state, waits at most `SUPPLIER_RECOVERY_COOLDOWN_MS=3000`, and receives one probe on each following item. A successful live capture closes the incident immediately.
- **Manual failure circuit:** authentication, missing credentials, CAPTCHA, missing/not-ready applications, and incompatible portal layouts open a permanent circuit for the current quotation. Product-specific uncertainty such as unresolved Santa Cruz stock blocks that item without disabling the supplier for unrelated medicines.
- **Diagnostic circuit breaker:** `npm run diagnose:live` shares the same incident policy as the application. Transient infrastructure failures remain eligible for half-open recovery; only failures requiring operator intervention skip the remaining checks.
...
- **Packaged diagnostic:** `scripts/live-diagnostic.mjs` is an explicit `electron-builder` input. `DATABASE_STARTUP_TIMEOUT_MS` bounds database startup to 2 minutes by default; promise rejections are logged, SQLite is closed, and Electron exits with code `1` instead of leaving an indefinite background process.
- **Windows application-control gate:** the generated portable executable is structurally validated, but an unsigned fresh hash can be blocked by Smart App Control (`Code Integrity` events `3033`/`3077`). Keep the Git/bootstrap route as the supported multi-PC deployment until release artifacts receive a trusted Authenticode signature; do not advise disabling Windows protection.
...
- **DM Playwright portability:** production packaging includes `playwright-core`, not a bundled browser. Windows Edge is the default `msedge` channel. Explicit `playwright` fails closed if Edge is missing; controlled `auto` may return to Electron only on that pre-interaction startup failure. This keeps the portable package smaller and avoids a machine-specific browser cache.
...
- **Live progress contract:** `main.js` emits sanitized `quote-progress` events for quote, item and supplier phases; `preload.js` exposes a removable listener; `quote-progress.js` reduces those events into deterministic UI state and percentage. Supplier rows must reflect real connector completion, failure, retry, recovery, empty response, block or timeout rather than estimated timers.
...
- **Santa Cruz operation guard:** the renderer disables quote submission while explicit preparation is active and repeats the same guard inside the handler. This prevents a quotation from competing for the global mutex and incorrectly losing Santa Cruz as `busy`.
...
- **Windows Packaging:** `npm run build` followed by `npm run package`. Output is written to `C:\dist-wimifarma`; `resources\santacruz-search.ps1` is an explicit extra resource and must match the source hash in release validation.
