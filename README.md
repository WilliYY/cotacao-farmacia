...
O diagnóstico ao vivo retorna erro quando qualquer fonte fica sem resultado confirmado, sem opção válida, com captura antiga ou com rótulo de preço diferente do contrato. Uma oscilação de rede não remove a distribuidora dos próximos medicamentos: depois do limite de falhas ela entra em recuperação e recebe uma tentativa curta de reentrada. Somente autenticação, CAPTCHA, aplicativo ausente ou mudança incompatível de tela bloqueiam os itens seguintes até intervenção. Correções linguísticas observadas durante uma busca não são aprendidas automaticamente: um apelido ou abreviação só entra na memória após o operador marcar o resultado como `APROVADO` na revisão manual. Alias oficiais e correções ortográficas únicas continuam disponíveis sem depender do histórico.
...
- **Falha de internet:** cada erro transitório recebe a repetição limitada configurada. Após três itens consecutivos com oscilação, a fonte entra em recuperação `half-open`, aguarda 3 segundos e recebe uma única tentativa nos itens seguintes; ao responder, volta automaticamente à cotação. Credencial recusada, CAPTCHA, aplicativo sem janela e layout incompatível exigem intervenção e permanecem bloqueados. Não há fallback para preço antigo.
...
- Toda nova cotação começa com as quatro distribuidoras desmarcadas. O operador escolhe somente as fontes desejadas, e a pesquisa fica bloqueada até pelo menos uma distribuidora ser selecionada.
- A tela inicial usa faixas funcionais e cores distintas para ANB, Profarma, Santa Cruz e DM Paraná, mantendo exemplos, fontes e ações legíveis em janelas grandes e compactas. O cartão preserva sua altura mínima e a área principal ganha rolagem quando necessário, evitando que a barra de pesquisa cubra avisos operacionais.
